...
static const struct file_operations cdrv_open_fops = {
...
static int chardev_registration(void)
{
	...
	res =
	    alloc_chrdev_region(&chardrv_dev_number, DRV_MINOR_START, DEV_COUNT,
				DEVICE_FILE);
	if (res) {
	...
	chardrv_devp =
	    kzalloc(DEV_COUNT * sizeof(struct chardrv_dev), GFP_KERNEL);
	if (chardrv_devp == NULL) {
	...
	for (i = 0; i < DEV_COUNT; i++) {
		if (!device_create
		     (chardrv_class, NULL,
		      MKDEV(MAJOR(chardrv_dev_number),
			  MINOR(chardrv_dev_number) + i), NULL, "%s.%d",
		      DRVNAME, i)) {
			pr_notice
