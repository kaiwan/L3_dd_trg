...
# Global settings
EV_CLASSES="-e block -e fs -e ext4 -e filemap -e vfs -e mmc -e hda -e scsi -e iommu -e pagemap"
...
taskset 01 trace-cmd record ${EV_CLASSES} -v -e *vm* \
...
which trace-cmd >/dev/null || {
  echo "$0: trace-cmd(1) not installed?"
  exit 1
}
lsmod |grep -q sbd || {
